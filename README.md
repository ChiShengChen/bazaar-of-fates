...
**Transits & progressions 行運與推運.** An **Overlay** selector adds an outer ring of either *transits* (today's/any-day sky) or *secondary progressions* (1 day = 1 year — the natal chart advanced in time, progressed Sun ~1°/yr); a **time slider** scrubs ±5 years and live-refreshes the ring via the LLM-free `/cast` (instant). **Major transits 重要行運**: a slow planet (Jupiter/Saturn) within 3° of a natal angle (ASC/MC/DSC/IC) is highlighted with a gold halo + line, **graded by potency** (conjunction > square; tighter orbs glow stronger) and by **phase** — solid ▸ *applying* (building) vs dashed ▹ *separating* (fading) — and carries the **exact-trigger date** (when the planet perfects the contact, found by zero-crossing search, retrograde-aware).
