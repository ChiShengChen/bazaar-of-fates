...
<p align="center">
  ...
  <br/>
  <em>natal wheel 本命星盤 · synastry bi-wheel 合盤雙輪 — see <a href="docs/astrology.md">docs/astrology.md</a> for the full visual guide</em>
</p>
...
**Annual report 年度報告.** For one person + a year, `/annual-report` assembles a cross-tradition forecast — the Western **Solar Return** (ascendant, highlights, the year's key transits), **BaZi 流年/大運**, **紫微 流年四化**, and **Jyotiṣa Mahādaśā** — and an LLM ties them into one bilingual year-ahead report. The UI's *Annual* mode renders it print-ready (Save as PDF).

**Export 匯出.** Download any star/rāśi/七政 wheel as PNG (SVG→canvas, dependency-free) or print any reading / annual report to PDF. Every system's guide in [docs/](docs/README.md) embeds a real screenshot of its chart.
...
| `POST` | `/cast/{system}` | deterministic chart, no LLM → `Chart`（query: `house_system`, `transits`, `transit_date`, `progress`, `progress_method`, `solar_return`, `lunar_return`）|
| `POST` | `/reading/{system}` | chart + bilingual reading → `Reading`（body: `focus`, `house_system`, `transits`, `transit_date`, `progress`, `progress_method`, `solar_return`, `lunar_return`）|
...
| `POST` | `/annual-report` | one birth + year → Solar Return + 八字流年/大運 + 紫微四化 + Jyotiṣa daśā + synthesis |
...
```bash
pytest -q     # 64 tests: every system casts; 6 house systems vs swisseph; transits +
