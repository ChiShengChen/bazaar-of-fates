...
| `POST` | `/cast/{system}` | deterministic chart, no LLM → `Chart`（query: `house_system`, `transits`, `transit_date`, `progress`, `progress_method`, `solar_return`）|
| `POST` | `/reading/{system}` | chart + bilingual reading → `Reading`（body: `focus`, `house_system`, `transits`, `transit_date`, `progress`, `progress_method`, `solar_return`）|
...
```bash
pytest -q     # 60 tests: every system casts; 6 house systems vs swisseph; transits +
              # applying/separating + exact dates + major-transit highlights; secondary &
              # solar-arc progressions + major progressions + directed-to-angles; Solar
              # Returns; aspect ranking; timelines; synastry / composite / Davison / returns;
